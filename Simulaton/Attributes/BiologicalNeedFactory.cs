...
namespace Simulaton.Attributes
{
    class BiologicalNeedFactory : NeedFactory
    {
        ...
        public Needs CreateNeeds(Simulator owner)
        {
            ...
            //Hunger
            float startingHunger = 0.6f + 0.3f * (float)randomizer.NextDouble();
            float startingRate = -0.03f * (float)randomizer.NextDouble();
            float hungerThreshold = 0.2f * (float)randomizer.NextDouble();
            ...
            float energyRate = -0.06f * (float)randomizer.NextDouble();
            ...
            ModifyNeedEffect decHealthMod = new ModifyNeedEffect(energy, health, -energyImpact, energyImportance, energyThreshold, ((x, y) => x < y));
            energy.AddEffect(decHealthMod);

            Need hunger = new Need(Need.ID_NOURISHMENT, startingHunger, startingRate);
            ModifyNeedEffect dmgHealthMod = new ModifyNeedEffect(hunger, health, hungerImpact1, hungerImportance, hungerThreshold, ((x, y) => x < y));
            ModifyNeedEffect dmgEnergyMod = new ModifyNeedEffect(hunger, energy, hungerImpact1, hungerImportance, hungerThreshold, ((x, y) => x < y));
            ModifyNeedEffect healthyMod = new ModifyNeedEffect(hunger, health, hungerImpact2, hungerImportance, 1 - hungerThreshold, ((x, y) => x > y));
